...
class ListNode:
    ...
    """Wrap the given value in a ListNode and insert it after this node. Note that this Node could already have a next node it is pointing to."""
    ...
    """Wrap the given value in a ListNode and insert it before this Node. Note that this Node could already have a previous Node it is pointing to."""
    ...
    """Rearranges this ListNode's prev and next pointers accordingly, effectively deleting this ListNode"""
    def delete(self):
        if self.prev:
...
class DoublyLinkedList:
    ...
    def delete(self, node):
        ...
        # if DLL is empty, there is nothing to delete, so return
        ...
        # if DLL has one element, remove it by setting head & tail pointers to None
        if self.head == self.tail:
            self.head = None
            self.tail = None
        
        # if node we are to delete is the head
        # set DLL head pointer to node.next
        elif self.head == node:
            self.head = node.next
            # this calls the delete from inside class ListNode
            node.delete()
        
        # if node we are to delete is the tail
        # set DLL tail pointer to node.prev
        elif self.tail == node:
            self.tail = node.prev
            # this calls the delete from inside class ListNode vs. self.delete which would call this delete function
            node.delete()
        
        # more than three nodes in our DLL and not head or tail, all we have to do is delete the connections
        else:
            node.delete()
